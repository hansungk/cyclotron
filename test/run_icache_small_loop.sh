...
ELF_PATH="test/performance-model-tests/binaries/icache_small_loop.elf"
HIT_CFG="test/performance-model-tests/configs/icache_small_loop_hit.toml"
MISS_CFG="test/performance-model-tests/configs/icache_small_loop_miss.toml"
...
if [[ "$hit_issued" -gt 0 && "$hit_issued" -eq "$hit_completed" && "$hit_hits" -eq "$hit_issued" && "$hit_misses" -eq 0 && "$miss_issued" -gt 0 && "$miss_issued" -eq "$miss_completed" && "$miss_misses" -eq "$miss_issued" && "$miss_hits" -eq 0 && "$miss_cycles" -gt "$hit_cycles" ]]; then
  echo "Icache small_loop check: PASS"
  echo "  hit_cycles=${hit_cycles}"
  echo "  miss_cycles=${miss_cycles}"
else
  echo "Icache small_loop check: FAIL"
