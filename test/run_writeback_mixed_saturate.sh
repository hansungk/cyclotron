...
CONFIG_PATH="test/performance-model-tests/configs/writeback_mixed_saturate.toml"
ELF_PATH="test/performance-model-tests/binaries/writeback_mixed_saturate.elf"
...
if [[ "$wb_issued" -gt 0 && "$wb_completed" -gt 0 && "$wb_qf" -gt 0 && "$gmem_completed" -gt 0 && "$smem_completed" -gt 0 && "$wb_completed" -ge "$gmem_completed" && "$wb_completed" -ge "$smem_completed" ]]; then
  echo "Writeback mixed_saturate check: PASS"
else
  echo "Writeback mixed_saturate check: FAIL"
