...
CONFIG_PATH="test/performance-model-tests/configs/smem_partial_conflict.toml"
ELF_PATH="test/performance-model-tests/binaries/smem_partial_conflict.elf"
LANES=16
BANKS=8
...
if [[ "$instructions" -gt 0 && "$active_lanes" -eq "$expected_active" && "$conflict_lanes" -eq "$expected_conflicts" && "$unique_banks" -eq "$expected_unique_banks" ]]; then
  echo "SMEM partial-conflict check: PASS"
else
  echo "SMEM partial-conflict check: FAIL"
