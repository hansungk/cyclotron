...
CONFIG_PATH="test/performance-model-tests/configs/smem_no_conflict.toml"
ELF_PATH="test/performance-model-tests/binaries/smem_no_conflict.elf"
LANES=16
...
expected_unique_banks=$((instructions * LANES))
...
if [[ "$instructions" -gt 0 && "$active_lanes" -eq "$expected_active" && "$conflict_lanes" -eq "$expected_conflicts" && "$unique_banks" -eq "$expected_unique_banks" ]]; then
  echo "SMEM no-conflict check: PASS"
else
  echo "SMEM no-conflict check: FAIL"
