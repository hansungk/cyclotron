...
CONFIG_PATH="test/performance-model-tests/configs/lsu_shared_ldq_fill.toml"
ELF_PATH="test/performance-model-tests/binaries/lsu_shared_ldq_fill.elf"
...
shared_ldq_issued="$(jq -r '.total.lsu_stats.shared_ldq_issued' "${RUN_DIR}/summary.json")"
...
if [[ "$lsu_issued" -gt 0 && "$shared_ldq_issued" -gt 0 && "$smem_issued" -gt 0 && "$shared_ldq_qf" -gt 0 && "$global_ldq_qf" -eq 0 && "$global_stq_qf" -eq 0 && "$shared_stq_qf" -eq 0 ]]; then
  echo "LSU shared_ldq_fill check: PASS"
else
  echo "LSU shared_ldq_fill check: FAIL"
  echo "  lsu_issued=${lsu_issued}"
  echo "  shared_ldq_issued=${shared_ldq_issued}"
