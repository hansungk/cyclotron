...
CONFIG_PATH="test/performance-model-tests/configs/lsu_ldq_fill.toml"
ELF_PATH="test/performance-model-tests/binaries/lsu_ldq_fill.elf"
...
global_ldq_issued="$(jq -r '.total.lsu_stats.global_ldq_issued' "${RUN_DIR}/summary.json")"
...
if [[ "$lsu_issued" -gt 0 && "$global_ldq_issued" -gt 0 && "$gmem_issued" -gt 0 && "$l0_accesses" -gt 0 && "$global_stq_qf" -eq 0 && "$shared_ldq_qf" -eq 0 && "$shared_stq_qf" -eq 0 ]]; then
  echo "LSU ldq_fill check: PASS"
  echo "  global_ldq_queue_full_rejects=${global_ldq_qf} (informational)"
else
  echo "LSU ldq_fill check: FAIL"
  echo "  lsu_issued=${lsu_issued}"
  echo "  global_ldq_issued=${global_ldq_issued}"
